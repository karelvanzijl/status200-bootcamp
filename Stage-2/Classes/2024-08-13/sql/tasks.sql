...
-- Voucher field is in percantages; value 10 represents 10% discount on the total order
...
-- List of all orders along with their total order values, applying any voucher discounts.
SELECT 
    o.order_id, 
    ROUND(SUM(od.quantity * od.price * (1 - o.voucher / 100)), 2) AS "Total Order Value"
FROM 
    orders AS o
JOIN 
    orderdetail AS od ON od.order_id = o.order_id
GROUP BY 
    o.order_id;

-- Retrieve all products that have been ordered at least twice
SELECT 
    p.prod_name AS "Product Name", 
    COUNT(od.quantity) AS "Number of Orders"
FROM 
    product AS p
JOIN 
    orderdetail AS od ON od.prod_id = p.prod_id
GROUP BY 
    p.prod_id
HAVING 
    COUNT(od.quantity) > 1;

-- Find all products with a unit price over 200 dollars that have been ordered by customers in Amsterdam or Rome
SELECT 
    p.prod_name AS "Product Name",
    p.unit_price AS "Price",
    c.cust_city AS "Customer City"
FROM 
    product AS p
JOIN 
    orderdetail AS od ON od.prod_id = p.prod_id
...
WHERE 
    c.cust_city IN ("Amsterdam", "Rome")
    AND p.unit_price > 200;

-- Count the number of sales per country
SELECT 
...
-- Retrieve the total value of orders per city where the total exceeds 1000 dollars, applying any voucher discounts.
SELECT 
    c.cust_city AS "City",
    ROUND(SUM(od.quantity * od.price * (1 - o.voucher / 100)), 2) AS "Total Order Value"
FROM 
...
GROUP BY 
    c.cust_city
HAVING 
    SUM(od.quantity * od.price * (1 - o.voucher / 100)) > 1000;
